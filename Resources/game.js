...
congratulationsImage.src = "images/gratz.png";
...
var update = function (modifier) {
  ...
  for (var i = 0; i < NUM_MONSTERS1; i++) {
    ...
    if (!monsters[i].isAlive) continue;
    if (
      Math.abs(hero.x - monsters[i].shootX) <= 20 &&
      Math.abs(hero.y - monsters[i].shootY) <= 20 &&
      monsters[i].shootY > hero.y
    ) {
      gameover(0);
    }
  }
  ...
  for (var i = 0; i < NUM_MONSTERS2; i++) {
    ...
    if (!monsters2[i].isAlive) continue;
    if (
      Math.abs(hero.x - monsters2[i].shootX) <= 20 &&
      Math.abs(hero.y - monsters2[i].shootY) <= 20 &&
      monsters2[i].shootY > hero.y
    ) {
      gameover(0);
    }
  }
...
var render = function () {
  ...
  if (heroReady) {
    ...
    if (hero.isShooting)
      if (heroshotReady) ctx.drawImage(heroshotImage, hero.shootX, hero.shootY);
  }
  for (var i = 0; i < NUM_MONSTERS1; i++) {
    if (a1Ready) {
      var tmp = false;
      if (monsters[i].isAlive) {
        ctx.drawImage(
          monsters[i].image ? a1Image : a2Image,
          monsters[i].x,
          monsters[i].y
        );
        if (monsters[i].isShooting)
          ctx.drawImage(shotImage, monsters[i].shootX, monsters[i].shootY);
  ...
  for (var i = 0; i < NUM_MONSTERS2; i++) {
    if (a3Ready) {
      ...
      if (monsters2[i].isAlive) {
        ctx.drawImage(
          monsters2[i].image ? a3Image : a4Image,
          monsters2[i].x,
          monsters2[i].y
        );
        if (monsters2[i].isShooting)
          ctx.drawImage(shotImage, monsters2[i].shootX, monsters2[i].shootY);
